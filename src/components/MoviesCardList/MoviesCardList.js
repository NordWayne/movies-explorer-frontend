...
const MoviesCardList = ({movies, savedMovies, handleSaveMovie, handleDeleteMovie, isLoading, countOfMovies, handleAddMovies}) => {
  ...
  if (!movies.length) return <span className='card__text'>Ничего не найдено</span>
  return (
    <>
    <ul className='card__list'>
      {location.pathname !== '/saved-movies' ?
        ...
        movies.map((movie) => <MoviesCard handleDeleteMovie={handleDeleteMovie} handleSaveMovie={handleSaveMovie} movie={movie} key={movie.id} movies={movies} savedMovies={savedMovies}/>)}
    </ul>
